...
pub struct U128MaxGenericHasher<H: HashAlgorithm> {
    pub padding: H::Padding,
    pub state: H,
    pub size: u128,
}

impl<H: HashAlgorithm> HasherPadOps for U128MaxGenericHasher<H> {
    fn size_mod_pad(&self) -> usize {
        (self.size & self.padding.last_index() as u128) as usize
    }
...
impl<H: HashAlgorithm + Default> Default for U128MaxGenericHasher<H> {
    fn default() -> Self {
        Self {
...
impl<H: HashAlgorithm> HasherContext for U128MaxGenericHasher<H> {
    type State = H;
...
impl<H: HashAlgorithm> Hasher for U128MaxGenericHasher<H> {
    fn finish(&self) -> u64 {
